...
### 3) 🧾 세출 산출내역 관리 (`#tab-expenditure`)
- 세출 예산액 vs 집행 실적 1:1 비교
- 잔액 및 집행률 자동 산출
- 산출내역별 매칭된 지출 건수 및 상세 내역 드릴다운
...
### 6) 📁 다중 사업 프로필 관리
- 단일 프로그램 내에서 여러 사업(예: `2026학년도 학교급식비`, `방과후학교 운영`, `현장체험학습`, `목적사업비`)을 독립적으로 생성/전환/백업/복원
- 프로필별 독립된 세입, 세출, 지출 실적 데이터 관리

### 7) 📥 엑셀 업로드 및 서식 다운로드
- **에듀파인 엑셀 일괄 임포트**: 지출결의 내역서 및 세입 엑셀 파싱하여 자동 등록
- **표준 엑셀 서식 다운로드**: 사용자가 양식에 맞춰 입력 후 업로드 가능하도록 샘플 엑셀 제공
- **데이터 백업/복원**: JSON 내보내기/불러오기 지원

### 8) 🖨️ A4 정산 보고서 인쇄 및 PDF 내보내기
- 결재 및 보관용 **A4 최적화 정산 총괄표 인쇄 스타일** 내장 (`@media print`)
- 불필요한 사이드바와 컨트롤을 제거하고 공문서 서식 형태로 즉시 출력

### 9) 📖 인터랙티브 사용자 매뉴얼 & 가이드 내장 (`#user-manual-modal`)
