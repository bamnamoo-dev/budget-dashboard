...
- **완벽한 2장 분할 레이아웃 (`break-before: page`)**:
  - **제 1장 (결재용 정산 총괄표)**: 공문서 상단 헤더 + **화면 그대로 1행 3열 KPI 요약 카드** + **예산세입세출 연동 정산표**가 **정확히 1장에 꽉 차게 피팅**
  - **제 2장 (시각화 분석 보고서)**: `break-before: page` 기반 **세입 vs 세출 항목별 실적 막대 차트** + **주요 재원 집행 상태 요약**이 2페이지에 2열 그리드로 큼직하게 배치
...
- 상단 헤더의 **[사용 설명서]** 버튼을 통해 언제든 열어볼 수 있는 5-탭 글래스모피즘 사용자 매뉴얼
  1. **🚀 3단계 빠른 시작 (Quick Start)**: 프로필 선택 ➡️ 엑셀 3종 다운 ➡️ 드래그앤드롭 적용 (신규 프로필 매핑 설정 팁 수록)
  2. **📥 K-에듀파인 엑셀 다운로드 상세 가이드**: 세입/세출/지출실적(원인행위) 메뉴 경로 및 기간(03.01~기준일) 팁
  3. **📊 정산 지표 & 잔액 공식 이해**: 예산상 잔액 vs 실제 정산 잔액(반납·이월 기준) 및 집행률 계산식
  4. **🔗 재원 매핑, 다중 사업 프로필 & 전체 백업/복원**: 목적사업비 1:1/1:N 매핑, 다른 PC/담당자 1초 JSON 백업 및 복원 가이드
  5. **🖨️ 결재용 A4 가로 인쇄, 엑셀 다운로드 & 데이터 보안**: 2장 분할 인쇄 옵션, 엑셀 다운로드, 100% 브라우저 인메모리 보안 원칙
